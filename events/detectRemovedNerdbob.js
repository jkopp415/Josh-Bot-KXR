...
const leaderboardFile = path.join(__dirname, 
    "../assets/nerdbob_leaderboard.json");

module.exports = {
    name: Events.MessageReactionRemove,
    async execute(reaction, user) {

        // If a reaction is added to a message from when before the bot was
        // activated, you must fetch the message
        if (reaction.partial) {
            try {
        ...
        // Get the name of the reaction emoji and the id of the message
        // it belongs to
        ...
        if (reactionName == nerdbobBotEmoji) {
            ...
            try {
                ...
                // If the receiver is already in the leaderboard, remove 1 from
                // their score. Otherwise, add them to the file
                if ((user = leaderboardData.nerdbobs.find
                        (i => i.userId === receiver)) != null) {
                    user.score -= 1;
                } else {
